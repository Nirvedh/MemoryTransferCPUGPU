...
int main(int argc, char* argv[]){
	...
	exampleStruct* example = (exampleStruct*) malloc (nBlocks*sizeof(exampleStruct));// Allocate CPU Memory 
	...
	exampleStruct* dexample;
	cudaMalloc(&dexample,nBlocks*sizeof(exampleStruct)); // Initalize GPU array
	cudaMemcpy(dexample,example,nBlocks*sizeof(exampleStruct),cudaMemcpyHostToDevice); // CPU->GPU
	...
	cudaMemcpy(example,dexample,nBlocks*sizeof(exampleStruct),cudaMemcpyDeviceToHost); // GPU->CPU
